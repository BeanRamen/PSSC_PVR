...
namespace lab2.exemple.Operations
{
  internal sealed class ValidateExamOperation : ExamOperation
  {
    ...
    protected override Exam.IExam OnUnvalidated(Exam.UnvalidatedExam unvalidatedExam)
    {
      (List<ValidatedStudentGrade> validatedGrades, IEnumerable<string> validationErrors) = ValidateListOfGrades(unvalidatedExam);

      if (validationErrors.Any())
      {
        return new Exam.InvalidExam(unvalidatedExam.GradeList, validationErrors);
      }
      else
      {
        return new Exam.ValidatedExam(validatedGrades);
      }
    }

    private (List<ValidatedStudentGrade>, IEnumerable<string>) ValidateListOfGrades(Exam.UnvalidatedExam examGrades)
    {
